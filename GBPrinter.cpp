...
GameBoyPrinter::GameBoyPrinter(int in, int out, int clock) {
  gb_in_ = in;
  gb_out_ = out;
  gb_clock_ = clock;
  pinMode(gb_out_,   OUTPUT);
  pinMode(gb_clock_, OUTPUT);
  pinMode(gb_in_,    INPUT);           // set pin to input
  digitalWrite(gb_in_, HIGH);       // turn on pullup resistors
  digitalWrite(gb_out_, HIGH);       // turn on pullup resistors
}

void GameBoyPrinter::printStatusCode(int result)
{
  if(result<0)
  {
...
uint8_t GameBoyPrinter::GBSerialOut(uint8_t command) {
  uint8_t gbReplyy=0;
  for (uint8_t clock=0;  clock<8;  ++clock) {
    gbReplyy <<= 1;			// Shift Printer gbReplyy
    digitalWrite(gb_clock_, 0);           // Send clock signal

    if((command << clock) & 0x80){
      digitalWrite(gb_out_, 1); // Write out to printer
    }
    else{ 
...
boolean GameBoyPrinter::GBPCommand(uint8_t command, uint16_t bodyLength) {
 ...
 */
uint16_t GameBoyPrinter::sendHeader(uint8_t commandCode,
                                    uint8_t compressionIndicator,
                                    uint16_t bodyLength)
{
...
void GameBoyPrinter::sendChecksum(uint16_t checksum)
{
  // LSB (send as two bytes)
  // 0000 0000
  // 0000 0000  
 ...
 */
boolean GameBoyPrinter::endData(uint16_t checksum)
{
 ...
 */
boolean GameBoyPrinter::sendPrint(uint8_t leftMargin, uint8_t rightMargin, uint8_t palette, uint8_t exposure)
{
  uint16_t CRC = 0;
  sendSync();
...
void GameBoyPrinter::sendBand()
{
  for(int i=0; i<2; ++i) sendStrip();
}

// 160 pixels wide / 8 = 20 tiles
void GameBoyPrinter::sendStrip()
{
...
// 8x8
void GameBoyPrinter::sendTestTile()
{
  for(int i=0; i<16;++i)
  {
    GBSerialOut(0xFF);
 ...
 */
boolean GameBoyPrinter::getAcknowledgement()
{
 ...
 */
uint8_t GameBoyPrinter::getStatusCode()
{
